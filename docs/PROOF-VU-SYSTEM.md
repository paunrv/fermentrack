...
Pendiente menor (no bloquea cierre VU): hex decorativos en paletas `COLORS` de cards brewer legacy (intencional). Ops/shell i18n residual ✅. `canvas-accents.ts` eliminado — chrome usa `--proof-accent` directo.
