...
export function downloadPedidoPreviewPdf(input: PedidoPreviewPdfInput): void {
  ...
  y += 8
  doc.text(`${L.orderDate}: ${fmtDate(input.fechaPedido, locale)}`, MARGIN_MM, y)
  y += 5
  ...
  for (const line of input.lineas) {
    ...
    doc.text(
      line.precioUnitario > 0 ? fmtMoney(line.precioUnitario, locale) : '—',
      colPrice,
      y,
      { align: 'right' }
    )
    doc.text(
      line.subtotal > 0 ? fmtMoney(line.subtotal, locale) : '—',
  ...
  doc.text(L.orderPreview, MARGIN_MM, y)
